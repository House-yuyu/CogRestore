...
from basicsr.archs.xrestormer_arch import TransformerBlock_X
...
# ==================== RIR with SSB ====================
class RCP_SSB(nn.Module):
    def __init__(self, n_feats, n_blocks):
        super(RCP_SSB, self).__init__()
        module_body = [SSB(n_feats) for _ in range(n_blocks)]
...
# ==================== Reflection Conv Block ====================
class R_convd(nn.Module):
    def __init__(self, inputchannel, outchannel, kernel_size, stride):
        super(R_convd, self).__init__()
...
# ==================== AtmosphericPriorRefiner ====================
class APR(nn.Module):
    def __init__(self, n_feats, out_ch=None, blocks=2):
        super(APR, self).__init__()
        self.match_channel = out_ch
        self.conv_init1 = R_convd(3, n_feats // 2, 3, 1)
        self.conv_init2 = R_convd(n_feats // 2, n_feats, 3, 1)
        self.extra = RCP_SSB(n_feats, n_blocks=blocks)
        if out_ch is not None:
            self.conv_out = R_convd(n_feats, out_ch, 3, 1)
...
class DecoderBlock(nn.Module):
    def __init__(self, in_dim, window_size, overlap_ratio,c_heads, s_heads, spatial_dim_head, ffn_expansion_factor, bias, LayerNorm_type, num_blocks,
                 prompt_dim=512, rcp_dim=48):
        super(DecoderBlock, self).__init__()
        self.norm1 = nn.LayerNorm(in_dim)
        self.norm2 = nn.LayerNorm(in_dim)
        self.cross_att = CrossAttention_LDM(query_dim=in_dim, context_dim=prompt_dim)
        # self.cross_att_img = CrossAttention_LDM(query_dim=in_dim, context_dim=prompt_dim)
        self.fuse_rcp = Modulator(inChannels=in_dim)
        self.XRestormerBlock = nn.Sequential(*[
            TransformerBlock_X(dim=in_dim, window_size=window_size, overlap_ratio=overlap_ratio, num_channel_heads=c_heads, num_spatial_heads=s_heads, 
    ...
    def forward(self, x, prompt, rcp_feature, iter):
        ...
        x = self.cross_att(self.norm1(x), prompt) + x
        ...
        return self.XRestormerBlock(x)


# ==================== Main Model: CogRestore ====================
class CogRestore(nn.Module):
    def __init__(self,
                 inp_channels=3,
                 out_channels=3,
                 dim=48,
                ...
                spatial_dim_head = 16,
                 ...
                 num_vector=8,
                 iter_times=2):
        super(CogRestore, self).__init__()
        ...
        self.atmospheric_extractor = ASPE(channels=3)
        ...
        self.encoder_level1 = nn.Sequential(*[
                        TransformerBlock_X(dim=dim, window_size=window_size, overlap_ratio=overlap_ratio[0], num_channel_heads=channel_heads[0], num_spatial_heads=spatial_heads[0], spatial_dim_head=spatial_dim_head, ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type)
            for _ in range(num_blocks[0])])
        
        self.down1_2 = Downsample(dim)
        self.encoder_level2 = nn.Sequential(*[
            TransformerBlock_X(dim=dim * 2, window_size=window_size, overlap_ratio=overlap_ratio[1], num_channel_heads=channel_heads[1], num_spatial_heads=spatial_heads[1], spatial_dim_head=spatial_dim_head, ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type)
            for _ in range(num_blocks[1])])
        
        self.down2_3 = Downsample(int(dim * 2))
        self.encoder_level3 = nn.Sequential(*[TransformerBlock_X(dim=int(dim * 4), window_size=window_size, overlap_ratio=overlap_ratio[2], num_channel_heads=channel_heads[2], num_spatial_heads=spatial_heads[2], spatial_dim_head=spatial_dim_head, ffn_expansion_factor=ffn_expansion_factor,
             bias=bias, LayerNorm_type=LayerNorm_type)
            for _ in range(num_blocks[2])])
        
        self.down3_4 = Downsample(int(dim * 4))
        self.latent = nn.Sequential(*[TransformerBlock_X(dim=int(dim * 8), window_size=window_size, overlap_ratio=overlap_ratio[3], num_channel_heads=channel_heads[3], num_spatial_heads=spatial_heads[3], spatial_dim_head=spatial_dim_head, ffn_expansion_factor=ffn_expansion_factor,
            bias=bias, LayerNorm_type=LayerNorm_type)
        ...
        self.decoder_level3 = DecoderBlock(in_dim=dim * 4, window_size=window_size, overlap_ratio=overlap_ratio[3], c_heads=channel_heads[2], s_heads=spatial_heads[2], 
        ...
        self.decoder_level2 = DecoderBlock(in_dim=dim * 2, window_size=window_size, overlap_ratio=overlap_ratio[2], c_heads=channel_heads[1], s_heads=spatial_heads[1],
                                           spatial_dim_head=spatial_dim_head, ffn_expansion_factor=ffn_expansion_factor, bias=bias,
                                           LayerNorm_type=LayerNorm_type, num_blocks=num_blocks[1] // 2)

        self.up2_1 = Upsample(int(dim * 2))
        self.decoder_level1 = DecoderBlock(in_dim=dim * 2, window_size=window_size, overlap_ratio=overlap_ratio[1], c_heads=channel_heads[0], s_heads=spatial_heads[0],
                                           spatial_dim_head=spatial_dim_head, ffn_expansion_factor=ffn_expansion_factor, bias=bias,
                                           LayerNorm_type=LayerNorm_type, num_blocks=num_blocks[0] // 2)

        self.refinement = nn.Sequential(*[TransformerBlock_X(dim=int(dim * 2), window_size=window_size, overlap_ratio=overlap_ratio[0], num_channel_heads=channel_heads[0], num_spatial_heads=spatial_heads[0], spatial_dim_head=spatial_dim_head, ffn_expansion_factor=ffn_expansion_factor,
            bias=bias, LayerNorm_type=LayerNorm_type)
            for _ in range(num_refinement_blocks)])
        self.output = nn.Conv2d(int(dim * 2), out_channels, 3, padding=1, bias=bias)

        # APR Extractor
        self.APR_extractor = APR(n_feats=dim)
        self.APR_down1 = Downsample(dim)
        self.APR_down2 = Downsample(dim * 2)
        self.APR_ch = nn.Conv2d(dim, dim * 2, 3, padding=1)
    ...
    def forward(self, imgs, texts):
        ...
        for i in range(self.iter_times):
            ...
            degware_vector = self.learnable_vector.repeat(lq_clip_feat.shape[0], 1, 1) + \
                             self.deg_aware(lq_clip_feat).unsqueeze(1).repeat(1, self.num_vector, 1)  # [B,N,512]
            ...
            if i == 0:
                ...
                rcp_feature = self.APR_extractor(ch_res_out)
                rcp_level1 = self.APR_ch(rcp_feature)
                rcp_level2 = self.APR_down1(rcp_feature)
                rcp_level3 = self.APR_down2(rcp_level2)
...
if __name__ == '__main__':
    ...
    net = CogRestore(model_clip=model, num_vector=8, num_blocks=[2,4,4,4], iter_times=2)
